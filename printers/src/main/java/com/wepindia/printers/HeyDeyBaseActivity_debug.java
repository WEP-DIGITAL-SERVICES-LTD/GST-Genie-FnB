...
public abstract class HeyDeyBaseActivity_debug extends WepBaseActivity implements View.OnClickListener {
    ...
    private static final String DEBUG_TAG = HeyDeyBaseActivity_debug.class.getSimpleName();
    public static final String ACTION_CONNECT_STATUS = "action.connect.status";
    private HeyDeyBaseActivity_debug.PrinterServiceConnection conn = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        printerUtil = new PrinterUtil(HeyDeyBaseActivity_debug.this);
        connection();
        registerBroadcast();
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                messageBox("Device found");
                //Device found
            }
            else if (BluetoothDevice.ACTION_ACL_CONNECTED.equals(action)) {
                messageBox("Device is now connected"); //Device is now connected
            }
                ...
                messageBox("Device has disconnected");
            }
        }
    };
        ...
        try{
            ...
            if (requestCode == REQUEST_ENABLE_BT) {
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    Intent intent = new Intent(HeyDeyBaseActivity_debug.this, BluetoothDeviceList.class);
                ...
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    if (CheckPortParamters(mPortParam)) {
                        ...
                        try{
                            ...
                            //BluetoothDevice dd = adapter.getRemoteDevice(mPortParam.getBluetoothAddr());
                            BluetoothDevice dd = adapter.getRemoteDevice(mPortParam.getBluetoothAddr());
                            int state = dd.getBondState();
                            System.out.println("device bond state "+state);
                            switch(state)
                            {
                                case BluetoothDevice.BOND_NONE : messageBox(dd.getName()+" is either off or idle");
                                    break;
                                case BluetoothDevice.BOND_BONDED : connectOrDisConnectToDevice();
                                    break;
                                case BluetoothDevice.BOND_BONDING : messageBox("Please wait ... while "+dd.getName()+" is getting paired");
                                    break;
                            }
    ...
    public void askForConfig() {
        ...
        }catch(Exception e)
        {
            ...
            Log.e("askforConfig",e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void connection() {
        conn = new HeyDeyBaseActivity_debug.PrinterServiceConnection();
        Intent intent = new Intent("com.gprinter.aidl.GpPrintService");
        intent.setPackage(this.getPackageName());
        bindService(intent, conn, Context.BIND_AUTO_CREATE); // bindService
    }

    public void checkPrinterConfig(boolean isErrorCorrection) {
        ...
        if (mPortParam.getBluetoothAddr().equalsIgnoreCase("") || state[printerNum] == false) {
            ...
            try {
                ...
                switch(status)
                {
                    case GpCom.STATE_NO_ERR : Log.i("Printi","STATE_NO_ERR");
                        break;
                    case GpCom.STATE_OFFLINE : Log.i("Printi","STATE_OFFLINE");
                        messageBox("printer is ofline");
                        break;
                    case GpCom.STATE_PAPER_ERR : Log.i("Printi","STATE_PAPER_ERR");
                        break;
                    case GpCom.STATE_ERR_OCCURS : Log.i("Printi","STATE_ERR_OCCURS");
                        break;
                    case GpCom.STATE_TIMES_OUT : Log.i("Printi","STATE_TIMES_OUT");
                        break;
                }
        ...
        public void onReceive(Context context, Intent intent) {
            if (ACTION_CONNECT_STATUS.equals(intent.getAction())) {
                ...
                if (type == GpDevice.STATE_CONNECTING) {
                    ...
                    messageBox("Unable to connect to printer");
                    return;
